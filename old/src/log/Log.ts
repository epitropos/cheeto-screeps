...
function color(str: string, textColor: string): string {
  return `<font color='${textColor}'>${str}</font>`;
}
...
export class LogImpl {
  ...
  public static loadSourceMap() {
    try {
      ...
      if (map) {
        LogImpl.sourceMap = new SourceMapConsumer(map);
      }
    } catch (err) {
  ...
  public level: number = LogLevels.NONE;
  public showSource: boolean = true;
  public showTick: boolean = true;
  ...
  public trace(error: Error): LogImpl {
